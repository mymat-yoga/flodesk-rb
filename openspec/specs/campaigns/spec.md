...
Campaign listing and Canva email publishing. Publishing is the highest-consequence operation in the client: it sends email to the subscriber list, so no response code — not a timeout, not a rate-limit rejection — justifies a retry.
