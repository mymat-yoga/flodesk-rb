...
Mapping Flodesk failures onto a rescuable error hierarchy. The API description declares no schema for any error response, so the observed `{code, message}` envelope must be parsed defensively and degrade gracefully. Also governs PII containment: nothing this gem logs or instruments may expose an email address, custom field value, or opt-in IP.
