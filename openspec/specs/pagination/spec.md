...
One uniform paging interface across list endpoints whose upstream parameter names disagree — `per_page` on most, `perPage` on workflows, PascalCase filters on campaigns. Also governs page metadata and the deliberately opt-in nature of full-collection traversal, which can consume an entire rate-limit budget.
