...
Connection handling for every Flodesk API call: HTTP Basic authentication, request identification, JSON encoding, timeouts, retry eligibility driven by per-endpoint idempotency declarations, and rate-limit visibility. Because the API offers no idempotency-key header, knowing which operations are naturally safe to repeat is the only mechanism available for safe retries.
